...
RUN_STGCN = False  # set False to only produce the LSTM half of the table

# ---------------------------------------------------------------------------
# CONFIG
# ---------------------------------------------------------------------------
LSTM_CSV = "posture_data_final.csv"       # x0..z32,label format
LSTM_MODEL_PATH = "posture_model_final.pth"  # or posture_model.pth, whichever you trained
LSTM_SEQ_LEN = 30

STGCN_DATA_DIR = "stgcn/data/new_data"       # class-named folders of .npy files
STGCN_MODEL_PATH = "stgcn/models/stgcn_best.pth"
STGCN_SEQ_LEN = 120
...
def main():
    ...
    X, y = make_sequences(landmarks, labels, LSTM_SEQ_LEN)
    ...
    def lstm_predict(window):
        with torch.no_grad():
            ...
            probs = torch.softmax(lstm_model(x), dim=1)
            return int(torch.argmax(probs, dim=1).item())

    preds, latencies = run_inference(lstm_predict, X_test)
    results.append(summarize("LSTM", y_test, preds, latencies))
    ...
    if RUN_STGCN:
        ...
        if len(X_stgcn) == 0:
            print(f"\nNo .npy files found under {STGCN_DATA_DIR} — skipping STGCN.")
        else:
            _, X_stgcn_test, _, y_stgcn_test = train_test_split(
                X_stgcn, y_stgcn, test_size=0.2, random_state=RANDOM_SEED, stratify=y_stgcn
            )
            ...
            stgcn_model.load_state_dict(torch.load(STGCN_MODEL_PATH, map_location="cpu"))
            stgcn_model.eval()

            def stgcn_predict(window):
                with torch.no_grad():
                    x = np.transpose(window, (2, 0, 1))
                    x = np.expand_dims(x, axis=-1)
                    x = np.expand_dims(x, axis=0)
                    x = torch.tensor(x, dtype=torch.float32)
                    probs = torch.softmax(stgcn_model(x), dim=1)
                    return int(torch.argmax(probs, dim=1).item())

            preds, latencies = run_inference(stgcn_predict, X_stgcn_test)
            results.append(summarize("STGCN", y_stgcn_test, preds, latencies))

    # ---------------- Compare ----------------
    if len(results) == 2:
        significance_note(results[0], results[1])

    print_results_table(results)

    print(f"\n{'='*70}\nCAVEAT FOR YOUR PAPER — state this explicitly\n{'='*70}")
    print("LSTM and STGCN were evaluated on DIFFERENT test sets of DIFFERENT sizes: "
          "LSTM's test set is drawn from the multi-subject, multi-angle combined "
          "dataset; STGCN's test set is drawn from its original single-angle, "
          "20-samples-per-class dataset. This comparison is preliminary until "
          "STGCN's data collection is expanded to match LSTM's (same subjects/"
          "angles/ages) — see collect_data_simple.py for the pattern to replicate "
          "for STGCN's 120-frame .npy format.")
