...
Architecture:
  • Background thread runs the exact detection pipeline from
    realtime_posture.py (MediaPipe pose → LSTM → personalised thresholds
    → optical-flow predictor), drawing overlays onto each frame.
  • Frames are served as an MJPEG stream at  /video_feed
  • Live state (class, metrics, alerts, flow, snooze) served as JSON
    at  /api/state   — the frontend polls it ~2×/s.
  • POST /api/snooze   → snooze alerts 5 min   (replaces the 'S' key)
  • POST /api/stop     → end session, finalise JSON log, return summary

Place next to: realtime_posture.py, optical_flow.py,
               posture_model.pth, user_baseline.json
...
def detection_loop():
    ...
    while not stop_event.is_set():
        ...
        if detected:
            ...
            label_text, label_color = "No person detected", (100, 100, 100)
            frame_buffer.clear()
            bad_start = moderate_start = None
        ...
        # ── Draw UI (same as realtime_posture.py) ─────────
        ...
        if ok:
            with lock:
                ...
                STATE.update({
                    ...
                    "cls": current_cls, "cls_name": label_text,
...
@app.route("/")
def index():
    return send_file("postureguard-monitor.html")
